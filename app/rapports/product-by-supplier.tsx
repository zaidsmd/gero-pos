...
export default function ProductBySupplierPage() {

    const {isLoading, isError, error, getProductBySupplier, productBySupplier} = useRapportsStore();

    const tableRef = useRef<HTMLDivElement>(null);
    const handlePrint = useCallback(() => printReport(tableRef, 'Produits par fournisseur'), []);
            ...
            <div className="flex items-center justify-between gap-3 mb-2">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-gray-200 text-sm text-[#3b5461] rounded-md hover:bg-gray-300 transition-colors"
                        onClick={handlePrint}
                        disabled={isLoading || !((productBySupplier.fournisseurs?.length || 0) > 0)}
                    >
                ...
                {isLoading ? (
                    ...
                    <table className="min-w-full table-fixed text-sm text-gray-700">
                        ...
                        <tr className="bg-primary text-left text-white sticky top-0 z-10 shadow-sm">
                            <th className="px-4 py-3 font-semibold tracking-wide rounded-tl-lg">Fournisseurs/Articles</th>
                            {productBySupplier.articles?.map((article, index) => (
                        ...
                        {productBySupplier.fournisseurs?.map((fournisseur, fournisseurIndex) => (
                            <tr key={fournisseurIndex}
                                className="odd:bg-white even:bg-gray-50 hover:bg-gray-100 transition-colors">
                                <th className="px-4 py-3 align-middle text-left whitespace-nowrap max-w-[18rem] truncate text-gray-900 font-medium">{fournisseur}</th>
                                {productBySupplier.articles?.map((article, articleIndex) => (
                                    <td key={articleIndex} className="px-4 py-3 align-middle text-center">
                                        {productBySupplier.data[fournisseur]?.[article] && (
                                            <div className="cell-content">
                                                <div className="quantity">
                                                    {productBySupplier.data[fournisseur][article].quantite > 0 ? productBySupplier.data[fournisseur][article].quantite : ""}
                                                </div>
                                            </div>
                                        )}
                                    </td>
                        ...
                        {
                            !productBySupplier?.fournisseurs?.length  && <tr>
                                <td colSpan={200} className="text-center p-4">Aucune donnée trouvée</td>
                            </tr>
                        }
